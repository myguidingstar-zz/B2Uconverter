...
_vietnamese_diacritics_removed = (
    u'aAaAdDeEoOoOuU'
...
def removeVietnameseDiacritics(str):
    global _vietnamese_diacritics, _vietnamese_diacritics_removed
    result = u''
    for c in str:
        idx = _vietnamese_diacritics.find(c)
        if idx >= 0:
            c = _vietnamese_diacritics_removed[idx]
        result += c
    return result
...
def convertVietnameseString(str, real, upper=False):
    ...
    while (i < len(str)):
        ...
        if _settings["RemoveDiacritics"]:
            segment = removeVietnameseDiacritics(segment)
